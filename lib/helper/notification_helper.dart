...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      // zone update: refresh zone data so widgets like BadWeatherWidget update
      if(message.data['type'] == 'zone_update'){
        try{
          if(Get.find<LocationController>() != null) {
            Get.find<LocationController>().updateZone();
          }
          // mark in shared prefs that this zone was updated by admin
          try{
            final address = AddressHelper.getAddressFromSharedPref();
            if(address != null && message.data['zone_id'] != null) {
              address.zoneUpdateAt ??= {};
              address.zoneUpdateAt![message.data['zone_id'].toString()] = DateTime.now().millisecondsSinceEpoch;
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      ...
      try{
        // If user opened app from a zone update notification, refresh zone data
        if(message.data['type'] == 'zone_update'){
          try{ Get.find<LocationController>().updateZone(); }catch(_){ }
          // mark in shared prefs that this zone was updated by admin
          try{
            final address = AddressHelper.getAddressFromSharedPref();
            if(address != null && message.data['zone_id'] != null) {
